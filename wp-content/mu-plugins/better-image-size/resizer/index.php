...
//					}
		...
		function get_attachment_picture($attachment_id = 0, $sizes = array(), $attr = array())
		{
			...
			// Handle picture class attribute
			$picture_class = isset($attr['picture_class']) ? 'class="' . esc_attr($attr['picture_class']) . '"' : '';
...
//						$avif_exists = file_exists($base_avif);
						$webp_exists = file_exists($base_webp);
//						if ($avif_exists) {
//							$avif_srcset = str_replace('.' . $extension, '.avif', $normalized_srcset);
//						}
						if ($webp_exists) {
							$webp_srcset = str_replace('.' . $extension, '.webp', $normalized_srcset);
						}
...
//							if ($avif_exists) {
//								$avif_srcset .= ' 1x, ' . str_replace('.' . pathinfo($retina['src'], PATHINFO_EXTENSION), '.avif', $retina['src']) . ' 2x';
//							}
							if ($webp_exists) {
								$webp_srcset .= ' 1x, ' . str_replace('.' . pathinfo($retina['src'], PATHINFO_EXTENSION), '.webp', $retina['src']) . ' 2x';
							}
							$srcset .= ' 1x, ' . $retina['src'] . ' 2x';
						}

//						if ($avif_exists) {
//							$html .= '<source media="(max-width:' . intval($breakpoint) . 'px)" srcset="' . $avif_srcset . '" type="image/avif">';
//						}
						if ($webp_exists) {
