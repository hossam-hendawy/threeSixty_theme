...
<section class="wp_collection_block" data-section-class="wp_collection_block">
  <div class="container">
    <h2 class="wp-collection-title has-label"><?= single_cat_title(); ?></h2>
    <?php if (category_description()) { ?>
      <div class="wp-collection-description paragraph"><?= category_description() ?></div>
    <?php } ?>
    <?php
    $paged = (get_query_var('page_val') ? get_query_var('page_val') : 1);
    $args = array('paged' => $paged, 'category_name' => $category->slug, 'posts_per_page' => 13, 'order' => 'DESC', 'orderby' => 'date', 'post_status' => 'publish');
