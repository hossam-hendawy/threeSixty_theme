...
<table class="form-table">
	...
	<tr valign="top" class="settings-row settings-row-cron-settings cron_opts cron"<?php echo $cron != 'cron' ? ' style="display:none"' : ''; ?>>
		...
		<td>
			...
			<div class="verified regular-cron-url"><a href="<?php echo esc_url( $cron_url ); ?>" class="external"><code id="copy-cronurl-1"><?php echo esc_url( $cron_url ); ?></code></a> <a class="clipboard" data-clipboard-target="#copy-cronurl-1"><?php esc_html_e( 'copy', 'mailster' ); ?></a></div>
			<div class="verified alternative-cron-url"><a href="<?php echo esc_url( $cron_url2 ); ?>" class="external"><code id="copy-cronurl-2"><?php echo esc_url( $cron_url2 ); ?></code></a> <a class="clipboard" data-clipboard-target="#copy-cronurl-2"><?php esc_html_e( 'copy', 'mailster' ); ?></a></div>
			<p><?php esc_html_e( 'or setup a crontab with one of the following commands:', 'mailster' ); ?></p>
			<ul>
			<li class="regular-cron-url"><code id="copy-cmd-1">wget -qO- '<?php echo esc_url( $cron_url ); ?>'</code> <a class="clipboard" data-clipboard-target="#copy-cmd-1"><?php esc_html_e( 'copy', 'mailster' ); ?></a></li>
			<li class="alternative-cron-url"><code id="copy-cmd-2">wget -qO- '<?php echo esc_url( $cron_url2 ); ?>'</code> <a class="clipboard" data-clipboard-target="#copy-cmd-2"><?php esc_html_e( 'copy', 'mailster' ); ?></a></li>
			<li class="regular-cron-url"><code id="copy-cmd-3">curl --silent '<?php echo esc_url( $cron_url ); ?>'</code> <a class="clipboard" data-clipboard-target="#copy-cmd-3"><?php esc_html_e( 'copy', 'mailster' ); ?></a></li>
	...
	<tr valign="top" class="settings-row settings-row-cron-lock">
		...
		<td>
			<?php if ( $last_hit && time() - $last_hit['timestamp'] > 720 && mailster( 'cron' )->is_locked() ) : ?>
				...
				<p><?php printf( esc_html__( 'Looks like your Cron Lock is still in place after %1$s! Read more about why this can happen %2$s.', 'mailster' ), '<strong>' . human_time_diff( $last_hit['timestamp'] ) . '</strong>', '<a href="' . mailster_url( 'https://kb.mailster.co/611bb5e36ffe270af2a9992b' ) . '" data-article="611bb5e36ffe270af2a9992b">' . esc_html__( 'here', 'mailster' ) . '</a>' ); ?></p>
				</div>
			<?php endif; ?>
	...
	<tr valign="top" class="settings-row settings-row-last-hit">
		...
		<?php else : ?>
			...
			(<a href="<?php echo mailster_url( 'https://kb.mailster.co/611bbaabf886c9486f8d9937' ); ?>" data-article="611bbaabf886c9486f8d9937"><?php esc_html_e( 'why?', 'mailster' ); ?></a>)</li>
		<?php endif; ?>
		</ul>
		</td>
	</tr>
